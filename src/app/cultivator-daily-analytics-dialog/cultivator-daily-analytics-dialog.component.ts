...
type Measure = 'distance' | 'avg_speed' | 'uptime' | 'area' | 'crops';
type Unit = 'metric' | 'imperial';


@Component({
	selector: 'app-cultivator-daily-analytics-dialog',
	templateUrl: './cultivator-daily-analytics-dialog.component.html',
	styleUrls: ['./cultivator-daily-analytics-dialog.component.css']
})
export class CultivatorDailyAnalyticsDialogComponent implements OnInit {
	...
	date_seq: string[];
	table_data_source!: MatTableDataSource<DailyAnalyticsData>;
	table_columns: string[];
	chart!: Chart;
	use_chart: boolean = true;
	use_metric: boolean = true;
	show_cultivator_selector: boolean = false;
	show_days_selector: boolean = false;

  	constructor(private fb: FormBuilder, private http: HttpClient, @Inject(MAT_DIALOG_DATA) public dialogData: DialogData, private dialogRef: MatDialogRef<CultivatorDailyAnalyticsDialogComponent>) { 
		...
		this.fc_cid = new FormControl('all');
		this.fc_days = new FormControl(3);

		this.table_columns = ['date', 'distance', 'avg_speed', 'uptime', 'area', 'crops'];
  	...
  	ngOnInit(): void {
		...
		this.createDialogTitle(this.dialogData.cid, this.dialogData.days);
		this.createDatasetToUpdateDialogContent(this.dialogData.cid, this.dialogData.days);
		...
		this.fc_cid.valueChanges.subscribe((cid: string) => {
			this.createDialogTitle(cid, this.fc_days.value);
			this.createDatasetToUpdateDialogContent(cid, this.fc_days.value);
		});

		this.fc_days.valueChanges.subscribe((days: number) => {
			days = days > 7 ? days : 7;
			this.createDialogTitle(this.fc_cid.value, days);
			this.createDatasetToUpdateDialogContent(this.fc_cid.value, days);
		});

	}

	createDialogTitle(cid: string, days: number): void {

		if (cid != 'all') {
			this.dialog_title = `Daily Performance of Cultivator ${cid}`;
			this.dialog_subtitle = `For past ${days} days`;
		} else {
			this.dialog_title = `Daily Performance Averaged over All Cultivators`;
			this.dialog_subtitle = `For past ${days} days`;
		}
	
	}

	createDatasetToUpdateDialogContent(cid: string, days: number): void {

		const promises_of_daily_analytics_data: Promise<DailyAnalyticsData>[] = [];
		
		if (cid != 'all') 
			for (let ds = days; ds > 0; ds--) {
				let ts_loc_begin: string = new Date(new Date().getTime() - (ds - 1)*24*3600*1000).toString().slice(4, 15);
				let ts_iso_begin: string = new Date(ts_loc_begin).toISOString();
				let ts_iso_end: string = new Date(new Date(ts_iso_begin).getTime() + 24*3600*1000 - 1000).toISOString();
				let headers: HttpHeaders = new HttpHeaders().set("Content-Type", "application/json");
				let join$ = forkJoin({
					date: of(ts_loc_begin),
					distance: this.http.post<number>(`${environment.api.url}/cultivators/${cid}/distance`, { ts_begin: ts_iso_begin, ts_end: ts_iso_end }, { headers: headers }).pipe(shareReplay()),
					avg_speed: of(null),
					uptime: of(null),
					area: of(null),
					crops: of(null)
				});
				promises_of_daily_analytics_data.push(firstValueFrom(join$) as Promise<DailyAnalyticsData>);
			}
		else 
			for (let ds = days; ds > 0; ds--) {
				let ts_loc_begin: string = new Date(new Date().getTime() - (ds - 1)*24*3600*1000).toString().slice(4, 15);
				...
				let join$ = forkJoin({
					date: of(ts_loc_begin),
					distance: this.http.post<number>(`${environment.api.url}/cultivators/${cid}/avg_distance`, { ts_begin: ts_iso_begin, ts_end: ts_iso_end }, { headers: headers }).pipe(shareReplay()),
					avg_speed: of(null),
					uptime: of(null),
					area: of(null),
					crops: of(null)
		...
		Promise.all(promises_of_daily_analytics_data).then((dataset: DailyAnalyticsData[]) => {

			this.table_data_source = new MatTableDataSource<DailyAnalyticsData>(dataset);

			this.chart.data.labels = dataset.map(dataObject => dataObject.date.slice(0, 6));
			...
			this.form.controls.measure.valueChanges.subscribe((measure: Measure) => {
				...
				this.chart.data.datasets[0].data = dataset.map(dataObject => dataObject[measure]);
				this.chart.update();
			});
			...
			this.fc_unit.valueChanges.subscribe((unit: Unit) => {
				if (unit == 'metric') {
					...
					if (this.form.controls.measure.value == 'distance' || this.form.controls.measure.value == 'avg_speed')
	...
	openDaysSelector() {
		const button: HTMLElement | null = document.getElementById("change-days-button");
		const rect: DOMRect | undefined = button?.getBoundingClientRect();
		const selector: HTMLElement | null = document.getElementById("days-selector");
		selector?.setAttribute("style", `position: fixed; left: ${rect!.right + 12}px; top: ${rect!.top}px; zIndex: 20;`);
		this.show_days_selector = true;
		setTimeout(() => { 
			this.show_days_selector = false; 
		}, 10000);
	}
